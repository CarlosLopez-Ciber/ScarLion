...
## Fase 1: Reconocimiento y Escaneo de Puertos
...
## Fase 2: Enumeración del Servicio Web y Detección de Vulnerabilidades
...
## Fase 3: Explotación de LFI para la Obtención de Credenciales
...
## Fase 4: Ruptura de Credenciales y Acceso Inicial
...
```bash
root:$1$gTqcQbju$b5huo6# WriteUp: Doctor | VulNyx
Como ahora nosotros podemos escribir en `/etc/passwd`, podemos reemplazar `x` con tu propia contraseña cifrada. Al hacer esto, el sistema ya no usará `/etc/shadow` para autenticar a `root`, sino que usará la contraseña que acabamos de adjuntar.
...
Ahora vamos a editar el archivo reemplazando la `x` en root:
...
root: dfde8cc67ed8819b2386dc74e472ecc6
user: 0819e6dfb35db7c61353e4dce311b397K8egw0UDboANeq/.:0:0:root:/root:/bin/bash
```
